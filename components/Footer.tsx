...
import { TG_HANDLE, MAX_URL } from "@/lib/contacts";
...
/*
  Путь `/maps/perm/` Яндекс отдавал 404 — города по слову в URL он не знает,
  ему нужен либо числовой id региона (50 — Пермский край), либо путь без
  города вовсе. Кнопка «Построить маршрут» из-за этого приводила человека
  на страницу ошибки. `rtt=auto` сразу выбирает маршрут на машине,
  пустая точка отправления перед `~` — «отсюда», её Яндекс берёт из геолокации.
*/
const ROUTE_URL =
 ...
 *
 * `pointer: coarse` — палец, а не мышь.
 */
...
function RouteButton() {
  // На сервере — «мышь»: разметка совпадёт с первым клиентским рендером,
  // а настоящий ответ подставится сразу после гидрации.
  const sameTab = useSyncExternalStore(
    subscribeCoarse,
